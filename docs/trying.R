...
pom_data4 <- read_csv2("pom_data4.csv") #%>%
 # lubridate::month(pom_data4$date)
...
ggplot(data = pom_data4, aes(x = month, y = pom, fill = area)) +
  geom_boxplot(outlier.shape = NA, notch = TRUE)+
  coord_cartesian( ylim = c(0, 1.2)) +
  labs(title = " monthly POM data",
       x = "Site",
       y = "POM content (g)") 
...
strftime(as.POSIXlt(pom_data4$date, format = "%Y-%d-%m"), format="%m")

pom <- lubridate::month(pom_data4, label = TRUE)
...
betterDates <- as.Date(pom_data4$date,
                       format = "%m/%d/%y")
betterDates
